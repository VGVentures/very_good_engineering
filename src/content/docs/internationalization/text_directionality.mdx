...
Flutter offers both visually and directionally-based versions of many related widgets and values. Visually-based values are defined in absolute directions such as `top`, `left`, `right`, and `bottom`. Directionally-sensitive widgets, by contrast, are defined in terms relative to the widget's directional alignment: `top`, `start`, `end`, `bottom`. 

This distinction is illustrated by comparing the `EdgeInsets` and `EdgeInsetsDirectional` classes. If I want to introduce a padding value that always comes before the beginning of a text widget, `EdgeInsetsDirectional` allows me to maintain that relative position regardless of the text's orientation:
```
Padding(
    padding: EdgeInsetsDirectional.only(start: 12),
    child: Text('Whether RTL or LTR text, padding will always be at the start of this string!'),
),
Padding(
