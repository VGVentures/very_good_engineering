...
Not only does the `GaugeGame` add the components to be displayed, it also handles the user input to update the game. As you'll see below, the `GaugeGame` can directly call methods on its components, and can also make its variables and methods availble to its children.
...
The `Gear` is a `PositionComponent` that renders the vehicle's current gear inside of a triagle. The `gearText` is set directly in the `GaugeGame`.
